...
$( document ).ready(function() {
  ...
  $(document.body).on(
    ...
    function () {
      ...
      if (wishStatus == "0" && $(this).hasClass("fav-sec")) {
        ...
        if ($(".fav-prod").length-1 <= 0) {
          $(this)
            .closest(".col-md--12")
            //commented and added by manikanta for favorites not displaying in next page
            // .append(
            //   '<div class="col-md-9 col-sm-9 col-lg-9 col-xs-12">No Products Found</div><br><br><br>'
            // );
            $(".progress").show();
            location.reload();
            //added by manikanta up to here
        }
   ...
   //added by manikanta for stoping no products when there are products in next page
   $(document).ready(function()
   {
     
     var procunt = parseInt($("#procunt").val());
     if(procunt <= 0){
       $("#noproducts").show();
     }
   });
  ...
  $(document.body).on(
    ...
    function () {
      ...
      if ($(this).hasClass("active")) {
		...
		//Commented to fix cart no. by Sagar on 01-31-2022
      ...
      $.get(siteURL + "/api/AddRemoveFromCart/" + wishStatus + "/" + pid).done(
        function (data) {
		  ...
		  //Added to fix cart no. by Sagar on 01-31-2022
		  if(wishStatus == 0)
		    var cart_count = parseInt($(".cart-block .round-circle").html()) - 1;
		else
		    var cart_count = parseInt($(".cart-block .round-circle").html()) + 1;
  ...
  $(document.body).on("click", ".addtocart", function () {
    ...
    if (wishStatus == 1) {
      ...
      $.get(siteURL + "/api/AddToCart/" + quantity + "/" + pid).done(function (
        ...
        //console.log(data);
		//Added to fix cart no. by Sagar on 01-31-2022
		var cart_count = parseInt($(".cart-block .round-circle").html()) + parseInt(quantity);
		$(".cart-block .round-circle").html(cart_count);
		//Added to fix cart no. by Sagar on 01-31-2022
  ...
  $(document.body).on("change", ".qnty_input", function () {
    ...
    var cart_item = $(this).attr("cart_item");
    //var row_select_class = ".cart-item-row-" + cart_item + " .select-quant"; //Commented & Added to fix Cart No.
	var scrn = $(this).attr("scrn");
    var row_select_class = ".cart-item-row-" + cart_item + " ."+scrn+" .select-quant";
  ...
  $(document.body).on("change", ".select-quant", function () {
    ...
    var cart_item = $(this).attr("cart_item");
     ...
     //var row_input_class = ".cart-item-row-" + cart_item + " .qnty_input"; //Commented & Added to fix Cart No.
	
	var scrn = $(this).attr("scrn");
    var row_input_class = ".cart-item-row-" + cart_item + " ."+scrn+" .qnty_input";
...
function isEmail(email) {
  ...
  if (token != "") {
    $.post(API_URL + "api/Login/ResetPassword", post_data).done(function (
      ...
      //console.log(data);
      //added by manikanta for reset password validation 
      if(data['message'] !='Password Reset Successfully!') {
          toast_message(data['message'],"E");
        }
        else{
      if (1) {
	...
	if (data.message.includes("again") || data.message.includes("expired"))
         ...
         //window.location.href = siteURL + "/login?returnURL=s";
         //added by manikanta for password reset is coming after refer again and again
		 toast_message("Password reset is succesful.");
     setTimeout(function(){
  ...
  $(document.body).on("click", ".tip", function () {
    ...
    if (tip == 3) {
      ...
      $.post(API_URL + "api/Cart/CartUpdate", cartDetails).done(function (
        ...
        //added by manikanta for reloading the page after adding tip
        location.reload();
    ...
    if (tip == 3) {
      ...
      if(tip_amount == ''){
        ...
        if($("#fstate").val() == '1'){
          ...
          //this commented by manikanta due to while adding tip from other the checkout button disabled up to total amount update
          //$("#checkout_submit").removeAttr("disabled");
        }
      }
      ...
      $.post(API_URL + "api/Cart/CartUpdate", cartDetails).done(function (
        ...
        //added by manikanta for reloading after adding tip from othertips
        if (tip_amount != "") {
          toast_message("Tip Applied.", "S");
          $(".progress").show();
          location.reload();
...
$(document).ready(function (){
	var payonline=document.getElementsByName("PaymentTypeId");
	for(var i=1; i<=payonline.length;i++)
	{
		var isCheck=$('#radio-' + i).is(":checked");
		var value=$('#radio-' + i).val();
		if(isCheck && value==8)
		{
			$(".payment-option-select .radio-custom").click();
		};
	};
});

// commented and added by manikanta for checkout button disable
...
$(".payment-option-select .radio-custom").on("click", function(){
  ...
  if($(".payment-option-select .radio-custom:checked").val() == '9' ){
     $('#checkout_submit').prop("disabled", false);
    } else if($(".payment-option-select .radio-custom:checked").val() == '2'){
       $('#checkout_submit').prop("disabled", false);
    }
  });
...
$(document.body).on("click", "#reorder_chk_ok", function () {
	window.location.href = '/';
});