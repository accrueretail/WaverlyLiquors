...
if(isset($routeParams['func'])){
    ...
    if($func  == 'setStore'){
        ...
        if($json_res->SessionId != ''){
            ...
            $curl = curl_init();
            $response = api_call($curl, $url, $data);
            $cart_json_res = json_decode($response);
            $products = $cart_json_res;
            $_SESSION['CartId'] = $products->CartId;
        }else{
    ...
    if($func == 'search'){
      ...
      if($func  == 'storetime'){
$store_day = json_decode($store_home_details);
       if($store_day->GetStoredetails->IsPickUp == 1){
     $today = date("l");
   $now_time = date("Hi");
 ...
 foreach($store_day->GetStoredetails->ListPickUpStoreTime as $storetime){
    if($storetime->DayID == $today){
$start = $storetime->StoreOpenTime;
$endt = $storetime->StoreCloseTime;
$day = $storetime->DayID;
    }
    }

$stt = date('Hi',strtotime($start));
$sct = date('Hi',strtotime($endt));
...
if($now_time  < $stt){
 $status = 'NO';
    ...
    //pagination
    if($func  == 'getPagination'){
        if(isset($routeParams['pid'])){
        $pageNo = $routeParams['status'];
        }else{
        $pageNo = $routeParams['category'];
        }
        $cat_id = $routeParams['pid'];
        //print_r($routeParams);
        //echo $pageNo;
        if($pageNo == 0){$pageNo='';}

        $url = API_URL.'api/Product/ProductGetList';
        $data = array("StoreId"=>$storeId,"AppId"=>$appId,"PageSize"=>12,"PageNumber"=>$pageNo,"IsClub"=>0,"CategoryId"=>$cat_id,"SessionId"=>$_SESSION['SESSION_ID'],"UserId"=>$_SESSION['USER_ID'],"SizeId"=>"","TypeId"=>"","VarietalId"=>"","CountryId"=>"","RegionId"=>"","IsFavorite"=>0,"IsFeatureProduct"=>1,"MaxPrice"=>0,"MinPrice"=>0,"KeyWord"=>"","DeviceId"=>$_SESSION['DEVICE_ID'],"DeviceType"=>"W");
        //print_r($data);
        $curl = curl_init();
        $response = api_call($curl, $url, $data);
        $json_res = json_decode($response);
